...
Les branches sont les **collections** du jeu. Elles n’ont pas de taille fixe : le regroupement `editorial-regroup-v1` a fusionné les 53 micro-collections de quatre pages en **17 collections larges** de 6 à 22 pages, en conservant les deux paliers (`base` = la version large de l’information, `full` = les détails qui achèvent le set). Le migrateur refuse un manifeste qui perdrait ou inventerait une page, archive les reçus des branches retirées dans le journal du joueur (`collection_migration`) et laisse l’évaluation suivante payer les nouvelles collections ; la Curiosité déjà gagnée n’est jamais reprise.
...
L’évaluation des paliers ignore les arbres dont la mère est absente. Elle ne renvoie donc aucune notification d’étape révélant leur contenu. La base exige toutes les pages de contexte ; le palier complet exige la base **et** toutes les pages précises. Les récompenses nominales dépendent de la taille de la collection : **75 × pages de base** pour le premier palier, **150 × pages de détail + 30 par page au-delà de quatre** pour le palier complet. Une grande collection demande plus de tirages, rapporte davantage en valeur absolue et offre un meilleur rendement par page ; le crédit reste tronqué au plafond de Curiosité et l’écart est annoncé (`nominal_reward`).
...
Il n’existe pas de comptes nominatifs, mot de passe, récupération de session, synchronisation interappareils ou administration des utilisateurs. Perdre le cookie empêche de retrouver directement la partie anonyme. `GET /api/export` produit désormais une sauvegarde JSON version 2 depuis un état serveur cohérent : solde, inventaire, tickets, compteur de boosters et reçus des paliers. La restauration remet ces reçus en place sans recalculer ni reverser les bonus. Ce fichier ne remplace pas une sauvegarde de la base pour les sessions, les journaux et le catalogue.
